...
- **Supplements are no longer single-blob SYNC anchors.** As of 2026-08-01, each entry in the `SUPPLEMENTS` array (in the `<script>` block, search for `const SUPPLEMENTS`) is a structured JS object instead of one `<!-- SYNC:supp-* -->` blob:
  `{ badge, badgeClass, name, topPick, topPickLink?, topPickLinkNote?, alternative?, why, dosing?, testing?, nickNote? }`
  — `topPick` is the recommended brand, `topPickLink`/`topPickLinkNote` are an optional affiliate link and its caption (e.g. "20% off with this link"), `alternative` is an optional other-brand string, `why` is the reasoning paragraph, `dosing`/`testing` are optional extra labeled sections (added for Magnesium — dosing amount, which lab test to ask a doctor for), `nickNote` is an optional personal aside. All are rendered in the modal (`openModal()`) as labeled blocks in that order, only when present. To update a supplement, edit its object directly in that array — there's no Obsidian-note sync for this section right now. If Nick wants Obsidian sync back for Supplements, the sync flow needs new instructions matching this field shape (and a matching structure in his Obsidian note) before it can work again.
- **Nick's own copy (the `why`/`dosing`/`testing`/product-description text he supplies) must be pasted in exactly as given — no rephrasing, typo fixes, or "cleanup."** He explicitly does not want any trace of AI polish in his writing. Factual corrections (e.g. a misspelled brand name that doesn't exist) are fine; touching his phrasing or sentence structure is not.
